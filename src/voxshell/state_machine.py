...
from voxshell.agent.executor import AgentExecutor, AgentResult
from voxshell.audio.capture import AudioCapture
...
from voxshell.tts.base import Speaker
from voxshell.utils.logging import log_state
from voxshell.vad.base import VoiceActivityDetector
from voxshell.wake.base import WakeWordDetector
...
class VoiceAssistantStateMachine:
    ...
    def run_once(self) -> PipelineResult:
        ...
        try:
            ...
            self._transition(State.LISTENING, "recording command")
            command_frames = self._record_command(frame_iter)
            self._transition(State.TRANSCRIBING, f"{len(command_frames)} frames")
    ...
    def _record_command(self, frame_iter: Iterator[AudioFrame]) -> Sequence[AudioFrame]:
        ...
        for frame in frame_iter:
            ...
            if is_speech:
                ...
                trailing_silence += 1
                recorded.append(frame)
                if trailing_silence >= self.config.end_silence_frames:
                    break
